...
/*
 * Leo Chen
 * based on Contest Management System (CMS) version
 *
 * Modifications for TIOJ support:
 *   - Write checker outcome to stdout and message to stderr
 *   - Adjust checker exit code
 *   - Adjust checker argument order
 *   - Interactors are not supported
 *   - Partial Score is not allowed in TIOJ
 */

/* NOTE: This file contains testlib library for C++.
 *
 *   Check, using testlib running format:
 *     check.exe <Output_File> <Input_File> <Answer_File> [<Result_File> [-appes]],
...
NORETURN void InStream::quit(TResult result, const char* msg)
{
    ...
    if (result == _ok) {
        ...
        std::fprintf(stdout, "4\n");
        std::fprintf(stderr, "Unexpected EOF");
    } else if (result == _points || result >= _partially){
        std::fprintf(stdout, "5\n");
        std::fprintf(stderr, "Partial Score is not allowed in TIOJ");
...
NORETURN void __testlib_help()
{
    ...
    std::fprintf(stderr, "    <output-file> <input-file> <answer-file> [<report-file> [<-appes>]]\n\n");
...
void registerTestlibCmd(int argc, char* argv[])
{
    ...
    {
        quit(_fail, std::string("Program must be run with the following arguments: ") +
            std::string("<output-file> <input-file> <answer-file> [<language> <source-code>]") +
            "\nUse \"--help\" to get help information");
    }
...
void registerTestlib(int argc, ...)
{
    if (argc  < 3 || argc > 5)
        quit(_fail, std::string("Program must be run with the following arguments: ") +
            "<output-file> <input-file> <answer-file> [<language> <source-code>]");
